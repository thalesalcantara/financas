...
def admin_dashboard():
    ...
    if True:
        ...
        if somente_pendentes and cooperado_id:
            ...
            pend_ids = {
                ...
                if item['restante'] > 0 and item['status'] in ('aberta', 'parcial', 'a_descontar')
            }
    ...
    if ajax_partial in ajax_financeiros:
        ...
        if ajax_partial == "resumo":
            coop_ids_rel = {c.id for c in cooperados} if cooperado_id else set()
            coop_ids_rel.update({getattr(l, "cooperado_id", None) for l in lancamentos if getattr(l, "cooperado_id", None)})
            coop_ids_rel.update({getattr(r, "cooperado_id", None) for r in receitas_coop if getattr(r, "cooperado_id", None)})
            coop_ids_rel.update({getattr(d, "cooperado_id", None) for d in despesas_coop if getattr(d, "cooperado_id", None)})
            coop_map_rel = {c.id: c for c in cooperados if c.id in coop_ids_rel}
            lanc_por_coop = defaultdict(list)
            for l in lancamentos:
                if getattr(l, "cooperado_id", None):
                    lanc_por_coop[l.cooperado_id].append(l)
            rec_por_coop = defaultdict(list)
            for r in receitas_coop:
                if getattr(r, "cooperado_id", None):
                    rec_por_coop[r.cooperado_id].append(r)

            for coop_id_rel in sorted(coop_ids_rel):
                coop = coop_map_rel.get(coop_id_rel)
                if not coop:
                    continue
                ...
                prod = sum((l.valor or 0.0) for l in lancs_coop)
                rec = sum((r.valor or 0.0) for r in recs_coop)
                inss4 = sum((l.valor or 0.0) * INSS_ALIQ for l in lancs_coop)
                sest05 = sum((l.valor or 0.0) * SEST_ALIQ for l in lancs_coop)
    ...
    # resumo por cooperado calculado no backend
    resumo_coop_rows = []
    resumo_totais = {
        "prod": 0.0, "inss4": 0.0, "sest05": 0.0, "rec": 0.0,
        "des": 0.0, "adiant": 0.0, "a_receber": 0.0, "saldo_pendente": 0.0,
