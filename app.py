...
def upload_escala():
    ...
    # =========================
    # SUBSTITUIR COMPLETAMENTE (não somar)
    # =========================
    try:
        # Conjuntos de escopo cobertos pelo arquivo importado
        def _norm_del(s: str) -> str:
            s = _u.normalize("NFD", str(s or "").strip().lower())
            s = "".join(ch for ch in s if _u.category(ch) != "Mn")
            return _re.sub(r"[^a-z0-9]+", " ", s).strip()
        ...
        plan_coop_ids: set[int] = set()
        plan_rest_ids: set[int] = set()
        plan_names_norm: set[str] = set()       # para cooperados sem ID
        plan_contratos_norm: set[str] = set()   # para quando não achou restaurante_id
        ...
        # 3) Apaga escalas antigas por nome de cooperado (sem ID), comparando normalizado
        ids_por_nome = set()
        if plan_names_norm:
            q = Escala.query.with_entities(Escala.id, Escala.cooperado_id, Escala.cooperado_nome)
            for _id, _cid, _nome in q:
        ...
        # Dica: mostra quantos estabelecimentos e cooperados foram afetados
        detalhes = []
        if plan_rest_ids:
        ...
        if plan_names_norm:
            detalhes.append(f"{len(plan_names_norm)} cooperado(s) por nome")
        if plan_contratos_norm:
