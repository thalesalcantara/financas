...
ADMIN_ABAS = {
    ...
    "coop_despesas": "Adiantamentos e Pendências",
...
def admin_dashboard():
    ...
    Removido do carregamento inicial:
    ...
    O resumo carrega somente os cards essenciais, com agregações SQL.
    As abas pesadas só carregam dados quando são abertas.
    """
    args = request.args
    active_tab = (args.get("tab") or "resumo").strip().lower()
    abas_removidas = {"receitas", "despesas", "coop_receitas", "beneficios", "sistemas"}
    if active_tab in abas_removidas:
        active_tab = "resumo"
    ...
    if getattr(admin_logado, "is_master", False):
        ...
        if active_tab == "config":
            ...
            return redirect(url_for("admin_dashboard", tab="resumo"))
        if active_tab != "resumo" and active_tab not in abas_liberadas:
    ...
    if ajax_partial in abas_removidas:
        ajax_partial = "resumo"
    ...
    # Listas básicas usadas em filtros e cadastros.
    cooperados = (
        Cooperado.query
        .join(Usuario, Cooperado.usuario_id == Usuario.id)
    ...
    lancamentos = []
    precisa_lancamentos_lista = active_tab in {"lancamentos", "folha"} or ajax_partial == "lancamentos" or bool(dows) or bool(considerar_periodo)
    if precisa_lancamentos_lista:
        lanc_base = q_lanc.order_by(Lancamento.data.desc(), Lancamento.id.desc()).all()
        if dows:
        ...
        total_producoes = float(q_lanc.with_entities(func.coalesce(func.sum(Lancamento.valor), 0.0)).scalar() or 0.0)
    ...
    # =========================
    # Despesas cooperado: mantém apenas adiantamentos e pendências.
    # =========================
    despesas_coop = []
    ...
    dq_periodo = DespesaCooperado.query
    if data_inicio and data_fim:
        dq_periodo = dq_periodo.filter(DespesaCooperado.data_inicio <= data_fim, DespesaCooperado.data_fim >= data_inicio)
    elif data_inicio:
        dq_periodo = dq_periodo.filter(DespesaCooperado.data_fim >= data_inicio)
    elif data_fim:
        dq_periodo = dq_periodo.filter(DespesaCooperado.data_inicio <= data_fim)
    if cooperado_id:
        dq_periodo = dq_periodo.filter(DespesaCooperado.cooperado_id == cooperado_id)

    total_adiantamentos_coop = float(
        dq_periodo.filter(DespesaCooperado.eh_adiantamento.is_(True))
        .with_entities(func.coalesce(func.sum(DespesaCooperado.valor), 0.0)).scalar() or 0.0
    )
    total_despesas_coop = float(
        dq_periodo.filter(or_(DespesaCooperado.eh_adiantamento.is_(False), DespesaCooperado.eh_adiantamento.is_(None)))
        .with_entities(func.coalesce(func.sum(DespesaCooperado.valor), 0.0)).scalar() or 0.0
    )
    total_adiantamentos_geral = float(
        DespesaCooperado.query.filter(DespesaCooperado.eh_adiantamento.is_(True))
        .with_entities(func.coalesce(func.sum(DespesaCooperado.valor), 0.0)).scalar() or 0.0
    )

    if active_tab == "coop_despesas" or ajax_partial == "coop_despesas":
        despesas_coop = dq_periodo.order_by(DespesaCooperado.data_fim.desc().nullslast(), DespesaCooperado.id.desc()).all()
        ...
        solicitacoes_adiantamento = adiantamentos_q.order_by(SolicitacaoAdiantamento.pedido_em.desc(), SolicitacaoAdiantamento.id.desc()).limit(200).all()
        adiantamento_status_map = {s.despesa_cooperado_id: s for s in solicitacoes_adiantamento if s.despesa_cooperado_id}

    resumo_coop_rows = []
    resumo_totais = {
        "prod": round(total_producoes, 2),
        "inss4": round(total_inss, 2),
        "sest05": round(total_sest, 2),
        "adiant_total": round(total_adiantamentos_geral, 2),
        "adiant": round(total_adiantamentos_coop, 2),
        "saldo_pendente": round(total_despesas_coop, 2),
        "des": round(total_despesas_coop, 2),
        "rec": 0.0,
        "a_receber": 0.0,
        "pend_programado": 0.0,
    }

    # Variáveis removidas: ficam zeradas para compatibilidade de template/rotas antigas.
    receitas = []
    despesas = []
    receitas_coop = []
    beneficios_view = []
    ...
    # Dados de outras abas só quando necessário.
    ...
    escala_hist_inicio = _parse_ymd_date(args.get("escala_hist_inicio")) or (current_date - timedelta(days=30))
    escala_hist_fim = _parse_ymd_date(args.get("escala_hist_fim")) or current_date
    trocas_hist_inicio = _parse_ymd_date(args.get("trocas_hist_inicio")) or (current_date - timedelta(days=30))
    trocas_hist_fim = _parse_ymd_date(args.get("trocas_hist_fim")) or current_date
    ...
    admins = Usuario.query.filter_by(tipo="admin", is_master=False).order_by(Usuario.id.asc()).all() if active_tab == "config" else []
    ...
    rendered = render_template(
        ...
        fast_mode=True,
        tab=active_tab,
        total_producoes=round(total_producoes, 2),
        ...
        total_adiantamentos_geral=round(total_adiantamentos_geral, 2),
    ...
    if ajax_partial:
        marker_map = {"resumo": "RESUMO", "lancamentos": "LANC", "coop_despesas": "COOP_DESPESAS"}
        marker = marker_map.get(ajax_partial)
        if marker:
            m = re.search(rf"<!--AJAX_{marker}_START-->(.*?)<!--AJAX_{marker}_END-->", rendered, flags=re.DOTALL)
            if m:
                return m.group(1)
    return rendered
...
def filtrar_lancamentos():
    ...
    flat = {}
    for k, vals in qs_args.items():
        if len(vals) == 1:
            flat[k] = vals[0]
        else:
            flat[k] = vals
    flat["tab"] = "lancamentos"
    return redirect(url_for("admin_dashboard", **flat))
