...
def admin_dashboard():
    args = request.args
    tab = args.get("tab", "lancamentos")

    # --- defaults (evita erro em template quando a aba não usa algo)
    lancamentos = []
    receitas = []
    despesas = []
    receitas_coop = []
    ...
    chart_data_lancamentos_coop = {"labels": [], "data": []}
    chart_data_lancamentos_cooperados = {"labels": [], "data": []}
    ...
    # outros blocos do template (mantém compatibilidade)
    beneficios_view = args.get("benef_tab", "ativos")
    historico_beneficios = []
    current_date = date.today()
    data_limite = current_date
    folha_inicio = None
    folha_fim = None
    folha_por_coop = {}
    trocas_pendentes = []
    trocas_historico = []
    trocas_historico_total = 0
    page_args = dict(args)
    ...
    # listas leves (usadas em filtros/seletores em várias abas)
    ...
    # =========================
    if tab in ("lancamentos", "resumo", "home"):
        q = Lancamento.query
        if restaurante_id:
        ...
        # limite/paginação para não travar o HTML
        page = args.get("page", type=int) or 1
        per_page = min(args.get("per_page", type=int) or 200, 500)

        pag = q.order_by(Lancamento.data.desc(), Lancamento.id.desc()).paginate(page=page, per_page=per_page, error_out=False)
        ...
        # totais (rápidos) — calcula apenas do que está na página (evita custo alto)
        ...
        # chart só quando pedir explicitamente (evita loop grande)
        if args.get("chart") == "1":
            sums = {}
            ...
            def _fmt_label(k: str) -> str:
                y, m = k.split("-")
                return f"{m}/{y}"
            chart_data_lancamentos_coop = {"labels": [_fmt_label(k) for k in labels_ord], "data": [round(sums[k], 2) for k in labels_ord]}

    # =========================
    # ABA: COOP (Receitas/Despesas institucionais)
    # =========================
    if tab in ("cooperativa", "receitas", "despesas"):
    ...
    # =========================
    # ABA: COOPERADOS (Receitas/Despesas cooperado)
    # =========================
    if tab in ("cooperados_fin", "cooperado_fin", "adiantamentos"):
        ...
        receitas_coop = rq2.order_by(ReceitaCooperado.data.desc(), ReceitaCooperado.id.desc()).limit(500).all()
        despesas_coop = dq2.order_by(DespesaCooperado.data_fim.desc().nullslast(), DespesaCooperado.id.desc()).limit(500).all()

        total_receitas_coop = float(sum((r.valor or 0.0) for r in receitas_coop))
        total_despesas_coop = float(sum((d.valor or 0.0) for d in despesas_coop if not d.eh_adiantamento))
        total_adiantamentos_coop = float(sum((d.valor or 0.0) for d in despesas_coop if d.eh_adiantamento))

    # =========================
    # ABA: DOCUMENTOS (carrega só aqui)
    # =========================
    if tab in ("documentos", "docs"):
    ...
    return render_template(
        ...
        trocas_historico_total=trocas_historico_total,
        page_args=page_args,
    )
...
def api_rest_avisos_unread_count():
    # endpoint usado pelo portal do restaurante; retorna 0 se você não implementou avisos
