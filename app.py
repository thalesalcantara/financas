...
def admin_dashboard():
    ...
    resumo_cooperados = cooperados
    if cooperado_id:
        resumo_cooperados = [c for c in cooperados if c.id == cooperado_id]
    else:
        resumo_ids = {getattr(l, 'cooperado_id', None) for l in lancamentos if getattr(l, 'cooperado_id', None)}
        resumo_ids.update({getattr(r, 'cooperado_id', None) for r in receitas_coop if getattr(r, 'cooperado_id', None)})
        resumo_ids.update({getattr(d, 'cooperado_id', None) for d in despesas_coop if getattr(d, 'cooperado_id', None)})
        try:
            dq_res = db.session.query(DespesaCooperado.cooperado_id).filter(DespesaCooperado.cooperado_id.isnot(None))
            if data_inicio and data_fim:
                dq_res = dq_res.filter(DespesaCooperado.data_inicio <= data_fim, DespesaCooperado.data_fim >= data_inicio)
            elif data_inicio:
                dq_res = dq_res.filter(DespesaCooperado.data_fim >= data_inicio)
            elif data_fim:
                dq_res = dq_res.filter(DespesaCooperado.data_inicio <= data_fim)
            resumo_ids.update({cid for (cid,) in dq_res.distinct().all() if cid})
        except Exception:
            pass
        if resumo_ids:
            resumo_cooperados = [c for c in cooperados if c.id in resumo_ids]
